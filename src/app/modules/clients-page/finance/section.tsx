...
import { TFinance } from "@/app/types/finance";

export default function FinanceSection() {
    return (
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
            {/* banner header */}
            <div className="text-center bg-sky-600 text-white text-3xl py-3 rounded-md font-semibold font-montserrat mb-10">We Are Strong In Repeat Orders Satisfaction Guaranteed</div>

            {/* Grid 2 kolom, semua card ditarik sama tinggi */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
                {Finance.map((item: TFinance) => (
                    <div key={item.bankName} className="flex flex-col h-full">
                        ...
                        <div className="bg-[#e6f4ff] rounded-md px-4 py-4 shadow-sm flex-1 flex items-center justify-center">
                            <p className="font-montserrat text-[20px] text-slate-800 leading-relaxed">{item.description}</p>
                        </div>
                    </div>
                ))}
            </div>
