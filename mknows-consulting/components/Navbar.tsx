  ...
  return (
    <Link
      ...
      className={`relative transition hover:opacity-85 ${
        active ? "text-brand-lime" : "opacity-90"
      }`}
      aria-current={active ? "page" : undefined}
    >
      {label}
      {active && (
        <span className="absolute -bottom-1 left-0 h-0.5 w-full bg-brand-lime" />
      )}
    </Link>
  );
}
      ...
      <div className="container-app flex h-20 items-center justify-between">
        ...
        <nav className="hidden items-center gap-15 md:flex">
          ...
          <div ref={moreRef} className="relative">
            <button
              type="button"
              className="relative transition hover:opacity-85 opacity-90 flex items-center gap-1"
